...
export default function RegistrationForm() {
  ...
  if (isSubmitted) {
    return (
      ...
      >
        ...
        <h2 className="text-3xl font-black text-slate-900 mb-4">Đã gửi đơn thành công!</h2>
        <p className="text-slate-600 mb-8 max-w-sm mx-auto font-medium">
          Cảm ơn bạn đã quan tâm. Ban chủ nhiệm CLB sẽ sớm liên hệ với bạn qua thông tin Zalo/Email đã cung cấp.
        </p>
        <button 
  ...
  return (
    <div className="bg-white p-8 md:p-12 rounded-[2.5rem] border border-slate-100 shadow-2xl relative overflow-hidden">
      ...
      <h2 className="text-3xl font-black text-slate-900 mb-2">Đơn đăng ký</h2>
      <p className="text-slate-500 mb-10 font-medium italic">Hãy điền thông tin của bạn để gia nhập Labcom</p>

      <form onSubmit={handleSubmit(onSubmit)} className="space-y-8">
        <div className="grid md:grid-cols-2 gap-6">
          <div className="space-y-2">
            <label className="text-sm font-black text-slate-700 uppercase tracking-widest pl-1">Họ và tên *</label>
            <input 
          ...
          <div className="space-y-2">
            <label className="text-sm font-black text-slate-700 uppercase tracking-widest pl-1">Mã sinh viên *</label>
            <input 
        ...
        <div className="grid md:grid-cols-2 gap-6">
          <div className="space-y-2">
            <label className="text-sm font-black text-slate-700 uppercase tracking-widest pl-1">Email liên hệ *</label>
            <input 
              type="email"
          ...
          <div className="space-y-2">
            <label className="text-sm font-black text-slate-700 uppercase tracking-widest pl-1">Số điện thoại (Zalo) *</label>
            <input 
              type="tel"
        ...
        <div className="space-y-4">
          <label className="text-sm font-black text-slate-700 uppercase tracking-widest pl-1">Bạn là sinh viên năm mấy? *</label>
        ...
        <div className="space-y-4">
          <label className="text-sm font-black text-slate-700 uppercase tracking-widest pl-1">Mục tiêu tham gia? *</label>
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            {goals.map((goal) => (
              <label key={goal} className="flex items-center gap-3 p-4 bg-slate-50 border-2 border-transparent rounded-2xl cursor-pointer hover:bg-slate-100 transition-all has-[:checked]:border-blue-600 has-[:checked]:bg-blue-50">
                <input type="checkbox" value={goal} {...register("goal")} className="w-5 h-5 rounded border-slate-300 text-blue-600 focus:ring-blue-500" />
                <span className="text-sm font-semibold text-slate-700 italic">{goal}</span>
              </label>
            ))}
          </div>
        </div>
        ...
        <div className="space-y-4">
          <label className="text-sm font-black text-slate-700 uppercase tracking-widest pl-1">Kinh nghiệm liên quan? *</label>
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            {experiences.map((exp) => (
              <label key={exp} className="flex items-center gap-3 p-4 bg-slate-50 border-2 border-transparent rounded-2xl cursor-pointer hover:bg-slate-100 transition-all has-[:checked]:border-blue-600 has-[:checked]:bg-blue-50">
                <input type="checkbox" value={exp} {...register("experience")} className="w-5 h-5 rounded border-slate-300 text-blue-600 focus:ring-blue-500" />
                <span className="text-sm font-semibold text-slate-700 italic">{exp}</span>
              </label>
            ))}
          </div>
        </div>
        ...
        <div className="space-y-2">
          <label className="text-sm font-black text-slate-700 uppercase tracking-widest pl-1">Bạn sẽ đóng góp gì cho CLB? *</label>
          <textarea 
        ...
        <div className="space-y-2">
          <label className="text-sm font-black text-slate-700 uppercase tracking-widest pl-1">Tải lên CV (PDF, tối đa 100MB)</label>
          <div className="relative group">
            ...
            <div className="flex flex-col items-center justify-center p-10 bg-slate-50 border-2 border-dashed border-slate-200 rounded-[2rem] group-hover:bg-blue-50 group-hover:border-blue-300 transition-all">
              ...
              <span className="text-sm font-bold text-slate-500">Kéo thả hoặc nhấn để tải lên</span>
              <span className="text-[10px] text-slate-400 mt-2 italic font-medium">Chỉ chấp nhận file PDF</span>
            </div>
          </div>
        </div>
